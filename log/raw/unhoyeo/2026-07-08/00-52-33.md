...
이 레포(ai-powered-teamwork)에서 앞으로 팀원 누구든 Claude Code를 켜서 작업할 때, main 브랜치가 아닌 새 브랜치(또는 워크트리)를 만들지 못하게 막고 싶다. 배경: 최근에 팀원(들)이 의도치 않게 워크트리를 5개나 만들어서 방치했고, 그중 일부엔 main에 없는 PLAN.md 설계 내용이 갇혀있다가 뒤늦게 발견해서 겨우 복구했다. 팀은 git 경험이 적은 비개발자 3인 + 멘토라, "항상 이 폴더에서 Claude Code를 켜고 main에서만 작업한다"가 이미 팀 방침(PLAN.md/CLAUDE.md/port_051/decisions.md에 이미 기록된 결정)인데 실제로 강제되지 않고 있다.

요청: PreToolUse 훅(또는 적절한 훅)을 settings.json에 추가해서, 브랜치를 새로 만드는 동작(예: git checkout -b, git switch -c, git branch <새이름>, EnterWorktree 도구 호출 등)을 자동으로 차단하고, 대신 "이 프로젝트는 항상 main 브랜치에서만 작업합니다"류의 안내 메시지를 보여주도록 설정해줘. 이 설정은 .claude/settings.json(팀 전체 공유, 레포에 커밋됨)에 넣어서 팀원 전체에게 적용돼야 한다 (개인 설정인 .claude/settings.local.json 말고).

주의: 이 레포는 이미 .claude/hooks/에 pre-prompt-pull.sh, session-start-reminder.sh, stop-log-push.sh 라는 기존 훅들이 있고, .claude/settings.json에 이미 UserPromptSubmit/SessionStart/Stop 훅이 등록돼 있을 것이다. 기존 훅 설정을 건드리지 말고 새 훅만 추가해줘.
...
지난번에 겪은 셸 문제는 훅 스크립트에는 영향 없을 거야(훅은 bash로 직접 실행돼서 이 세션의 zsh 별칭 문제를 안 거침). 안전하게 하나씩 만들고 테스트해서 적용할게.
...
All 8 test cases behave correctly. Now let's wire these into `.claude/settings.json`, merging with the existing hooks.



이제 JSON 유효성과 스키마를 검증할게.





실제로 이 세션에서 훅이 발동하는지 직접 확인해볼게 (테스트용 브랜치 생성 시도).





훅이 실제로는 안 막았네 — 테스트 브랜치가 만들어졌어. 바로 지우고 원인을 설명할게.





훅 스크립트 자체는 정상 동작(가짜 입력으로 직접 테스트했을 때 차단/허용이 다 맞게 나옴)하는데, **지금 이미 켜져 있는 세션은 설정을 다시 안 불러와서** 방금 테스트에서 실제로는 안 막혔어. 새로 추가된 훅은 세션을 새로 열거나 `/hooks` 메뉴를 한 번 열어야 적용돼 — 이건 나뿐 아니라 이미 켜져 있는 팀원들의 세션에도 똑같이 해당돼.

일단 이 설정을 커밋해서 팀 전체에 공유할게.
...
훅이 실제로 작동 중이라 이제 커밋 메시지 안의 예시 텍스트까지 걸러버리네 — 훅 로직을 고쳐야겠어. 파일 편집 도구로 수정하고, 테스트는 파일을 통해 진행할게(Bash 명령줄에 직접 위험 문구를 안 넣도록).

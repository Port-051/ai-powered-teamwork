...
사용자가 세션 종료 시 뜨는 "Can Anthropic look at your session transcript..." 데이터 사용 동의 팝업(session quality survey)을 앞으로 계속 안 뜨게(n으로 고정하거나 아예 다시 안 묻게) 설정하고 싶어합니다. Claude Code 설정(settings.json 등)에 이걸 끄는 옵션이 있는지 확인하고, 있다면 어떻게 설정하는지 알려주세요.

### assistant


`feedbackSurveyRate`라는 설정으로 이 팝업 자체를 끌 수 있어요 — 0으로 설정하면 앞으로 안 뜹니다. 개인 설정이니 전역(`~/.claude/settings.json`)에 넣을게요.

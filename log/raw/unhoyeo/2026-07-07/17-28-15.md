...
이 레포(ai-powered-teamwork)에서 백그라운드 세션이 자동으로 워크트리에 격리되지 않도록, .claude/settings.json에 "worktree": {"bgIsolation": "none"} 설정을 추가해줘. 이유: 이 프로젝트는 팀원들이 한 명씩 이 폴더에서 Claude Code를 켜고 항상 main 브랜치에 바로 커밋/푸시하는 단일 브랜치 워크플로우이고, log/raw/ 자동 push 훅이 항상 main 체크아웃을 전제로 짜여 있어서 워크트리 격리가 오히려 로그 자동화를 깨뜨림.
...
이 설정 자체를 바꾸려면 지금 가드 때문에 또 워크트리가 필요한 상황이라(닭이 먼저냐 달걀이 먼저냐), 이번만 딱 워크트리를 잠깐 썼다가 바로 main에 반영하고 나올게. 이후부터는 이 가드가 안 걸릴 거야.

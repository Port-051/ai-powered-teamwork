...
팀원이 자체 호스팅 메신저에서 AI를 멘션하면, 공개 API로만 연결된 별도 프로세스인 우리 원본 레이어(①메신저 어댑터 ②팀 협업 지능 레이어 ③개인↔팀 지식 브릿지)가 요청을 처리한다. 그 아래에서는 개인 학습 엔진(Hermes 포크)이 세션을 넘는 기억·사용자 모델링·자기개선을 담당하는 3단 구조다.

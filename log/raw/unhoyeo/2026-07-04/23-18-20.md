...
Add a Stop hook to this project's .claude/settings.local.json (or settings.json, whichever is appropriate for a personal/local preference not meant to be shared with the SW Maestro team via git) that runs `/opt/homebrew/bin/terminal-notifier -title "Claude Code" -message "응답 완료" -activate com.googlecode.iterm2 -sound Ping` every time Claude Code finishes responding (Stop event), so the user gets a macOS notification that switches focus to iTerm2 when clicked. This replaces relying on Claude remembering to run this command manually each turn, which has been unreliable.
...
Good, it's already gitignored globally. Now let's pipe-test the notification command and add the hook.



Command works. Now adding the Stop hook to settings.local.json.

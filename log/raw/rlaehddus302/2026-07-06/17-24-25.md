...
전역 설정(~/.claude/settings.json)의 env에 CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=1 을 추가해서, 세션 끝에 뜨는 "이번 세션 어땠나요 0~4" 만족도 설문을 끈다

### assistant
Read → merge → write. 전역 설정 파일을 먼저 읽을게요.



`env` 키가 아직 없으니 추가만 하면 돼요. 기존 설정은 그대로 두고 병합합니다.





설문 끄기 완료. 전역 설정(`~/.claude/settings.json`)의 `env`에 `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=1`을 넣었어요. **다음에 Claude Code를 새로 켜는 순간부터** 그 "0~4 평가" 창은 안 뜹니다 (지금 세션엔 이미 로드된 설정이라 재시작 후 적용). 기존 model·statusLine·theme 설정은 그대로 뒀어요.

이제 질문 두 개에 답할게요. 둘 다 사실 하나로 이어지는 얘기예요.

---

## 1. "붙이는 레이어"까지 포함한 로드맵

앞에서 2단계(붙이기)를 짧게만 짚었는데, **여기가 진짜 실력이 붙는 곳이자 뒤에서 물어본 취업 효용의 핵심**이라 제대로 펼칠게요. 앞 로드맵의 0~1단계(개념 + 부품 3개 만져보기)가 끝났다는 전제 위에 얹는 트랙입니다.

붙이는 레이어 = **①어댑터 + ②팀 집계 + ③권한 브릿지**. 이걸 만들려면 아래 5가지를 순서대로 배우고 만들어봐야 해요. 각 항목 옆에 **"이게 백엔드에서 뭐라고 불리는지"**를 같이 적어둘게요 (뒤 취업 얘기랑 연결됨):

| 순서 | 배울 것 | 구체적으로 하는 일 | 백엔드 용어로는 |
|---|---|---|---|
| **1** | **API 통합 실전** | Mattermost 이벤트 수신 → 처리 → 응답 왕복을 실제 코드로. 토큰 인증, 재시도, 에러 처리 포함 | REST/WebSocket 클라이언트, 외부 연동 |
| **2** | **어댑터 설계** | "Mattermost 데이터 모델"과 "Hermes/Honcho 데이터 모델"을 서로 변환하는 통역 계층. 관심사 분리 | 어댑터 패턴, 계층 분리(layered architecture) |
| **3** | **데이터·상태 모델링** | 메모리 하나하나에 `소유자·가시성` 태그 붙이고, "팀 공유 합집합"을 뽑는 조회 설계 | 데이터 모델링, 집계 쿼리 |
| **4** | **권한·인가 설계** ★ | 개인 지식이 팀으로 올라갈 때 거르는 필터, 비밀정보 마스킹, 접근 제어 규칙 | **Authorization(인가) / 접근제어** — 백엔드 핵심 |
| **5** | **시스템 설계** | 별도 프로세스 분리(라이선스 때문에 필수), 여러 팀원 동시 사용 시 상태 관리, 로깅·테스트 | 서비스 간 통신, 동시성, 운영 |

핵심은 **4번(권한·인가)**이에요. "개인 것 중 무엇을, 누구에게, 어떤 조건으로 팀에 공개하느냐"를 설계하는 건 여러분 제품에서 제일 novel한 부분이자, **동시에 모든 백엔드 서비스의 심장(로그인·권한·접근제어)과 똑같은 문제**예요. 여기를 제대로 파면 "오픈소스에 어댑터 붙였어요"가 아니라 **"개인↔팀 지식의 인가 경계를 설계했어요"**라는 이야기가 됩니다.

**학습 방식**: 1→5 순서대로, 각 단계를 "읽기"가 아니라 "제일 작은 걸 실제로 동작시키기"로. 예를 들어 4번은 처음부터 완벽한 필터 말고, "비공개 채널 대화는 팀 검색에 안 뜬다" 딱 한 규칙부터 코드로 만들어보는 식.

---

## 2. Java Spring 백엔드 취업에 효용이 있나? — 현실적으로

솔직하게 갈게요. **반은 맞고 반은 아니에요.** 냉정하게 나눠보면:

### ❌ 직접적으로 안 되는 것 (기대하면 안 되는 부분)
- 이 프로젝트 스택은 **Python + TypeScript**예요. Spring 취업의 핵심 무기인 **Spring Boot / JPA / Spring Security 포트폴리오는 이걸로 안 쌓여요.**
- 5개월 Python 코드를 쓴다고 Java 문법·Spring 생태계 숙련도가 늘지 않습니다. 그건 별도로 해야 해요.
- 즉 이 프로젝트가 **"Spring 준비"를 대체해주지는 않습니다.** 대체재 아님.

### ✅ 그런데 진짜로 되는 것 (여기가 핵심)
붙이는 레이어에서 배우는 **1~5번은 언어를 안 가려요.** 이건 "Python 스킬"이 아니라 **"백엔드 엔지니어의 사고방식" 그 자체**예요:

- **API 통합·외부 연동** → Spring 백엔드가 매일 하는 일
- **인가/접근제어 설계**(4번) → Spring Security로 하는 바로 그 일. 언어만 다르지 개념은 100% 동일
- **데이터 모델링·집계** → JPA로 하는 그 일
- **시스템 설계**(프로세스 분리, 동시성, 서비스 간 통신) → 백엔드 면접에서 프레임워크 문법 다음으로 제일 많이 파는 부분

면접에서 "권한 경계를 어떻게 설계했나요?"에 **실제로 만들어본 이야기**로 답하는 사람과, 책으로만 아는 사람은 완전히 다릅니다. 이 프로젝트는 그 이야기를 줍니다.

### 🎯 그리고 무시하면 안 되는 것: SW Maestro 자체
- 한국 취업 시장에서 **소마 수료 + 실사용자 있는 완성 제품 + 오픈소스 공개**는 언어와 무관하게 강력한 신호예요. "Java 프로젝트 하나 더"보다 이게 이력서에서 더 세게 먹힐 때가 많습니다.

### 💡 효용을 극대화하는 현실적 지렛대 하나
라이선스 제약 때문에 **여러분이 만드는 붙이는 레이어는 반드시 "별도 프로세스로 API만 호출"해야** 해요 (PLAN 6-1). 그런데 **별도 프로세스는 언어를 안 가립니다.** Hermes가 Python이어도, 그 위 팀 레이어(어댑터·인가·집계)는 이론상 **Java/Spring Boot로 짜도 똑같이 동작**해요 — Mattermost REST랑 Honcho API만 부르면 되니까.

즉 원하면 **"내가 맡은 팀 레이어 일부를 Spring으로 구현"**하는 선택지가 실제로 열려 있어요. 그러면 Python 팀 프로젝트 안에서 **진짜 Spring 백엔드 포트폴리오**를 뽑을 수 있습니다.
단, 이건 공짜가 아니에요 — 팀에 언어가 하나 더 늘고(빌드·배포·협업 비용), 지금 역할 분담(신홍재=팀 레이어 TS)과 조율이 필요합니다. **팀과 상의해야 할 결정**이에요.

### 📌 현실적 결론
> **"이 프로젝트만으로 Spring 취업 준비 끝"은 아니다. 하지만 붙이는 레이어는 언어 무관한 백엔드 실력(특히 인가·시스템 설계)을 실전으로 주고, 소마 + 완성 제품이라는 강한 이력 신호가 붙는다. Spring 문법·프레임워크 공부는 따로 병행해야 한다.**

가장 남는 장사는 이거예요:
1. **이 프로젝트**로 → "시스템 설계 + 인가 + 실사용 제품 출시" 스토리 확보
2. **따로 Spring 사이드 프로젝트/CS 기초**로 → 프레임워크 숙련도 채우기
3. 여력 되면 **팀 레이어 일부를 Spring으로** → 위 둘을 하나로 합치기

둘 중 하나만 하면 반쪽이고, 병행하면 "설계할 줄 아는 Spring 백엔드"라는, 신입 중 드문 포지션이 됩니다.

---

원하면 위 3번(팀 레이어 일부를 Spring으로 빼는 게 현실적으로 가능한지)을 아키텍처 관점에서 더 구체적으로 따져드릴게요 — 어느 부분을 Java로 떼어내면 팀에 부담이 적은지까지요.
